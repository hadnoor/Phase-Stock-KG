...
class Trans(nn.Module):
    ...
    def __init__(self, W, T, D_MODEL, N_HEAD, ENC_LAYERS, DEC_LAYERS, D_FF, DROPOUT, USE_POS_ENCODING = False, USE_GRAPH = False, HYPER_GRAPH = True, USE_KG = True, NUM_NODES = 87, config=None, ENCODER_LAYER = 'transf', USE_RELATION_KG = False):
        ...
        if self.encoder_architecture == 'transf':
            ...
            self.transformer_decoder_first = nn.TransformerDecoder(decoder_layer, num_layers=2)
            encoder_layer = nn.TransformerEncoderLayer(d_model=D_MODEL, nhead=N_HEAD, dim_feedforward=D_FF,batch_first=True)
            self.transformer_encoder_first = nn.TransformerEncoder(encoder_layer, num_layers=1)  #3,4
        else:
        ...
        if self.use_graph:# == True:
            if self.is_hyper_graph:
                ...
                #             #(Dropout(p=0.5), 'x -> x'),
                #             (MixHopConv(20, 64), 'x, edge_index -> x1'),  # Change parameters according to your configuration
                #             nn.ReLU(inplace=True),
                #             # (MixHopConv(64 , 64), 'x1, edge_index -> x2'),  # Adapt the input dimension to match the previous layer's output dimension multiplied by the number of heads
                #             # nn.ReLU(inplace=True),
                #             nn.Linear(192, SEC_EMB*2),  # Adapt the input dimension to match the previous layer's output dimension multiplied by the number of heads
                #         ])
                ...
                # self.graph_model = Sequential('x, edge_index, batch', [
                #             #(Dropout(p=0.5), 'x -> x'),
                #             (GCNConv(20, 64), 'x, edge_index -> x1'),  #change according to edge dimension
                #             nn.ReLU(inplace=True),
    ...
    def forward(self, xb, yb=None, graph=None, kg=None, tkg=None, relation_graph = None):
        ...
        if self.encoder_architecture == 'transf':
            ...
            # dec_out = self.transformer_decoder_fi
                # self.graph_model = Sequential('x, edge_index, batch', [
                #             #(Dropout(p=0.5), 'x -> x'),
                #             (MixHopConv(20, 64), 'x, edge_index -> x1'),  # Change parameters according to your configuration
                #             nn.ReLU(inplace=True),
                #             (MixHopConv(64 * 8, 64), 'x1, edge_index -> x2'),  # Adapt the input dimension to match the previous layer's output dimension multiplied by the number of heads
                #             nn.ReLU(inplace=True),
                #             nn.Linear(64 * 8, SEC_EMB*2),  # Adapt the input dimension to match the previous layer's output dimension multiplied by the number of heads
                #         ])rst(tgt=x_t , memory= x_enc).mean(dim=1)
        ...
        # print("price pred shape",price_pred.shape)
